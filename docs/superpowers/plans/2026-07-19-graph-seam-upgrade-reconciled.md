...
## Reconciliation deltas (engine shapes vs the frozen brief)

The engine is the source of truth; its ts-rs bindings are vendored. The UI adopts these shapes:

| Element | Brief (old) | Engine (target — use this) |
|---|---|---|
| `GraphScope` | struct `{ focus: string \| null; depth: number }` | **tagged enum** `{ type: "full" } \| { type: "focused"; path: string; depth: number }` |
| `SuggestedEdge` | `{ from, to, score }` | `{ from, to, weight, why }` |
| `GetSuggestions` | stub, no args | takes a `SuggestionScope`, returns real edges |
| `GraphNode.mtime_secs` | `u64` → `number` | `i64` → **`bigint`** in TS unless the engine annotates `#[ts(type="number")]` |
| extra | — | engine also ships `GraphAt`, `GraphDiff` (+ `GraphDelta`) — additive, DEFERRED this pass |

**Engine ask filed in parallel (not a blocker):** annotate `mtime_secs` as `#[ts(type="number")]`. Until then, coerce `Number(node.mtime_secs)` at the seam boundary (store/graphData) — a `bigint` timestamp does not `JSON.stringify` cleanly and cannot mix with `number` math.

**VERIFY AT RE-VENDOR (do not assume):**
1. Does the engine `GraphNode` include a `title` field? The brief assumed `{ path, title, degree, tags, mtime_secs }`. #109 confirms `degree`+`tags`. If `title` is ABSENT, drop every `n.title` reference and use `stem(n.path)` for the label; adjust the guard + tests accordingly.
2. Exact `SuggestionScope` shape (likely mirrors `GraphScope` full/focused — confirm from the generated binding).
3. Exact `QueryResponse` variant names + fields for the graph and suggestions responses (`type: "graph"` / `type: "suggestions"` assumed — confirm).
4. Whether `mtime_secs` landed as `bigint` or `number` (drives the coercion in Tasks 4/5).

**Goal:** Evolve the existing `react-force-graph-2d` graph view onto the engine seam contract (`GetGraph { scope: GraphScope }` → `Graph { GraphNode[], GraphEdge[] }`), moving neighborhood scoping server-side, adopting server degree/title/tags, adding recency encoding, defaulting to a local/focus view, and scaffolding inert seams for suggested edges + temporal.
...
- **BLOCKED ON ENGINE #109 + RE-VENDOR.** The Track-1 seam already merged (#106/#107), but engine PR #109 (`feat(graph): enrich GraphNode with degree and tags`) is still OPEN as of 2026-07-19. Do not start Task 1+ until #109 merges, bindings are re-vendored from `tau-rs/cairn` `main`, and Task 0 is complete.
...
**Interfaces:** (copy the GENERATED shapes verbatim; the below is the expected target — reconcile against the actual binding)
- Produces (consumed everywhere downstream):
  - `GraphScope = { type: "full" } | { type: "focused"; path: string; depth: number }` (tagged enum — NOT the brief's `{focus,depth}` struct)
  - `GraphNode = { path: string; title?: string; degree: number; tags: string[]; mtime_secs: number | bigint }` (`title` per VERIFY #1; `mtime_secs` type per VERIFY #4)
  - `GraphEdge = { from: string; to: string }`
  - `SuggestedEdge = { from: string; to: string; weight: number; why: string }` (NOT `{score}`)
  - `SuggestionScope` — verify shape (VERIFY #2; likely full/focused like `GraphScope`)
  - `QueryResponse` gains `{ type: "graph"; nodes: GraphNode[]; edges: GraphEdge[] }` (replacing `nodes: string[]`) and `{ type: "suggestions"; suggestions: SuggestedEdge[] }` — confirm variant names (VERIFY #3)
  - `Query` gains `{ type: "get_graph"; scope: GraphScope }`, `{ type: "get_suggestions"; scope: SuggestionScope }`, and the deferred temporal queries (`graph_at` / `graph_diff` — mock as unsupported this pass)
...
In `web/src/client/mock.test.ts` (scope is the TAGGED ENUM — `{type:"full"}` / `{type:"focused",path,depth}`):

```ts
it("get_graph returns GraphNode metadata and honors focused scope", async () => {
  const c = mockClient({
    "a.md": "# A\n[[b]]",
    "b.md": "# B\n#topic\n[[c]]",
    "c.md": "# C",
  });
  const full = await c.runQuery({ type: "get_graph", scope: { type: "full" } });
  expect(full.type).toBe("graph");
  expect(full.nodes.map((n) => n.path).sort()).toEqual(["a.md", "b.md", "c.md"]);
  expect(full.nodes.find((n) => n.path === "b.md")).toMatchObject({
    degree: 2, tags: ["topic"], // + title:"B" only if VERIFY #1 confirms a title field
  });
  const local = await c.runQuery({ type: "get_graph", scope: { type: "focused", path: "a.md", depth: 1 } });
  expect(local.nodes.map((n) => n.path).sort()).toEqual(["a.md", "b.md"]); // depth-1 from a
});

it("get_suggestions is empty; deferred temporal query is unsupported", async () => {
  const c = mockClient({ "a.md": "# A" });
  expect(await c.runQuery({ type: "get_suggestions", scope: { type: "full" } }))
    .toEqual({ type: "suggestions", suggestions: [] });
  // GraphAt/GraphDiff are deferred — mock rejects until a later pass wires them.
  await expect(c.runQuery({ type: "graph_at", revision: "HEAD", scope: { type: "full" } }))
...
Expected: FAIL. Then in `web/src/client/mock.ts`, replace the `case "get_graph"` to: build `GraphNode[]` (degree from undirected edge count, `title` from first `# heading` else stem **only if VERIFY #1 confirms a title field**, `tags` from the existing tag parse, `mtime_secs` deterministic — 0 or a per-note counter; emit as `bigint` if the binding is `bigint`), then switch on the tagged scope: `scope.type === "focused"` → BFS `scope.depth` hops (undirected) from `scope.path` and slice nodes+edges to the reached set; `scope.type === "full"` → return everything. Add `case "get_suggestions"` → `{ type: "suggestions", suggestions: [] }` (ignore the incoming `scope`) and `case "graph_at"` (and `graph_diff` if present) → throw an `unsupported` error mirroring the engine's `Unsupported`.
...
- Produces:
  - `interface GNode { id: string; label: string; degree: number; tags: string[]; mtimeSecs: number }` (`mtimeSecs` is always a JS `number` — coerce `Number(n.mtime_secs)` at the boundary so `bigint` bindings don't leak downstream)
  - `interface GLink { source: string; target: string; kind: "explicit" | "suggested"; weight?: number; why?: string }`
...
describe("buildGraphData", () => {
  ...
  it("merges suggested edges as kind:suggested with weight, dropping out-of-scope endpoints", () => {
    ...
    const suggested = [
      { from: "a.md", to: "b.md", weight: 0.8, why: "co-cited" },
      { from: "a.md", to: "zzz.md", weight: 0.9, why: "shared tag" }, // endpoint not in node set → dropped
    ];
    const { links } = buildGraphData(nodes, [], suggested);
    expect(links).toEqual([{ source: "a.md", target: "b.md", kind: "suggested", weight: 0.8, why: "co-cited" }]);
  });
  ...
  it("adjacency ignores suggested edges", () => {
    const nodes = [gn("a.md", 1), gn("b.md", 1)];
    const { links } = buildGraphData(nodes, [{ from: "a.md", to: "b.md" }], [{ from: "a.md", to: "b.md", weight: 1, why: "x" }]);
...
export function buildGraphData(
  ...
  const sugg: GLink[] = suggested
    ...
    .map((s) => ({ source: s.from, target: s.to, kind: "suggested" as const, weight: s.weight, why: s.why }));

  const gnodes: GNode[] = nodes.map((n) => ({
    id: n.path,
    label: (n.title ?? "") || stem(n.path), // if VERIFY #1 says no title field, use stem(n.path) only
    degree: n.degree,
    tags: n.tags,
    mtimeSecs: Number(n.mtime_secs), // coerce bigint|number → number (safe for Unix seconds)
  }));
...
Note: this consumes `s.weight`/`s.why` per the reconciled `SuggestedEdge`. If the vendored type differs (VERIFY at re-vendor), adjust field names to match the generated binding and update the tests — do not invent a field the contract lacks.
...
it("loadGraph local scope queries GetGraph with focused scope on activePath, depth 1", async () => {
  const calls: any[] = [];
  ...
  expect(calls).toContainEqual({ type: "get_graph", scope: { type: "focused", path: "a.md", depth: 1 } });
});

it("loadGraph({focus:null}) queries the global overview", async () => {
  // ...expect the emitted scope to be { type: "full" }
});
...
async loadGraph(opts) {
  const s = get();
  // Resolve focus/depth: explicit opts win; else local view keyed on graphFocus ?? activePath.
  const focus =
    opts && "focus" in opts ? opts.focus ?? null : s.graphFocus ?? s.activePath;
  const prevDepth = s.graphScope?.type === "focused" ? s.graphScope.depth : 1;
  const depth = opts?.depth ?? prevDepth;
  const revision = opts && "revision" in opts ? opts.revision ?? null : s.graphRevision;
  // Build the TAGGED-ENUM scope: null focus → full overview, else focused subgraph.
  const scope: GraphScope =
    focus == null ? { type: "full" } : { type: "focused", path: focus, depth };
  ...
  try {
    ...
    if (s.showSuggestions) {
      try {
        // SuggestionScope shape is VERIFY #2 — mirror the graph scope (full/focused).
        const sres = await client.runQuery({ type: "get_suggestions", scope });
        if (token !== seq.graph) return;
...
- New props (from EditorPane, Task 8):
  ```ts
  {
    ...
    scope: GraphScope | null;          // tagged enum: {type:"full"} | {type:"focused",path,depth}
...
```ts
const isGlobal = props.scope?.type === "full" || props.scope == null;
const capped = useMemo(() => {
  if (!props.graph) return { nodes: [], edges: [], truncated: false, total: 0 };
  // Global view (scope.type === "full") gets the perf cap; focused views are already bounded.
  return isGlobal
...
- [ ] **Step 2: Local/global toggle drives scope.** The segmented control's selected state comes from `props.scope?.type === "focused"` (local) vs `"full"`/null (global) — reuse the `isGlobal` memo. `onClick` calls `props.onSetScope(isGlobal ? props.activePath : null)` (going local re-scopes to the active note; going global passes `null`). Default-local + no-focus→global is enforced by the store/EditorPane (Task 8) passing `focus = activePath` initially and falling back to `{ type: "full" }` when `activePath == null`.
...
- Reconciliation items (see the deltas table at top): `GraphScope` tagged enum (Tasks 0, 5, 6), `SuggestedEdge {weight,why}` (Tasks 0, 4), `SuggestionScope` arg to `get_suggestions` (Tasks 0, 5, 11), `mtime_secs` bigint coercion (Tasks 0, 4), `GraphNode.title` presence (VERIFY #1 — Tasks 0, 4). Deferred/additive: `GraphAt`/`GraphDiff`/`GraphDelta` stay inert (Tasks 0, 6, 11).
- Out-of-scope endpoint drop for suggested edges: Task 4 test covers it.
